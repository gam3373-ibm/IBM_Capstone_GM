...
for i in range(0,len(launch_sites_df)):
    folium.map.Marker([launch_sites_df.iloc[i]['Lat'], launch_sites_df.iloc[i]['Long']], icon=DivIcon(icon_size=(20,20),icon_anchor=(0,0), 
...
#Add empty new column to spacex_df for the 
...
#Note to myself: Need to use site_map2 in the next section
...
launch_coast = [[launch_site_lat, launch_site_lon], [coastline_lat, coastline_lon]]
lines=folium.PolyLine(locations=launch_coast, weight=1)
...
# Create and add a folium.Marker on your selected closest coastline point on the map
# Display the distance between city and launch site using the icon property 
...
# Create a `folium.PolyLine` object using the coastline coordinates and launch site coordinate
launch_city = [[launch_site_lat2, launch_site_lon2], [city_lat, city_lon]]
lines2=folium.PolyLine(locations=launch_city, weight=1)

site_map2.add_child(distance_marker2)
site_map2.add_child(lines2)
